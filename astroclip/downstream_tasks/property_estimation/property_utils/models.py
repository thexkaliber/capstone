...
def few_shot(
    ...
    if model_weights_path is None:
        ...
        for epoch in range(max_epochs):
            ...
            for i, data in enumerate(train_loader, 0):
                inputs, labels = data
                optimizer.zero_grad()
                outputs = model(inputs.to("cuda:1")).squeeze()
    ...
    model.eval()
    with torch.no_grad():
        preds = model(torch.tensor(X_test, dtype=torch.float32).to("cuda:1")).squeeze().cpu().numpy()
    return model, preds


def zero_shot(
    X_train: ndarray, y_train: ndarray, X_test: ndarray, n_neighbors: int = 64, model_weights_path: str = None
) -> ndarray:
    """Train a zero-shot model using KNN"""
    if model_weights_path:
        neigh = KNeighborsRegressor(weights="distance", n_neighbors=64)
        neigh = joblib.load(model_weights_path)

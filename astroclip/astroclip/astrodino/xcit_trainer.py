...
from astroclip.models.xcit.ssl_meta_arch import SSLMetaArch
from dinov2.utils.config import custom_setup
from dinov2.utils.utils import CosineScheduler
from fvcore.common.checkpoint import PeriodicCheckpointer
from omegaconf import OmegaConf

from astroclip.astrodino.data.augmentations import DataAugmentationAstroDINO
from astroclip.astrodino.data.loaders import make_data_loader, make_dataset, SamplerType
from astroclip.astrodino.utils import MetricLogger
from astroclip.env import format_with_env
from torch.nn.utils import clip_grad_norm_

from astroclip.models.xcit import *
...
os.environ["WANDB_SERVICE_PORT"] = "8080"
...
def do_test(cfg, model, iteration):
    ...
    #         iterstring,
    #     )
    eval_dir = os.path.join("/workspace/astroclip/results", "outputs", "astroclip_image", wandb.run.id, "eval", str(iteration))
...
def do_train(cfg, model, run_name, group_name, resume=False):
    ...
    checkpointer = Checkpointer(
        model,
        f"/workspace/astroclip/results/astroclip_image/{run_name}",
        optimizer=optimizer,
        save_to_disk=True,
    )
    ...
    logger.info(f"Resuming from checkpoint at iteration {start_iter}")
    ...
    # if global_rank == 0:
    wandb.init(
        project="astrodino-xcit",
        entity="entity-name",
        name=run_name,
        group=group_name,
        resume="allow",
        dir=f"/workspace/astroclip/results/astroclip_image",
    ...
    for data in metric_logger.log_every(data_loader, 25, header, max_iter, start_iter):
        assert max_iter > start_iter, "max_iter must always be greater than start_iter"
        ...
        if fp16_scaler is not None:
            if cfg.optim.clip_grad:
                ...
                for v in model.student.values():
                    clip_grad_norm_(v.parameters(),cfg.optim.clip_grad)
...
def main_cli(cli_args=None):
    ...
    args.output_dir = f"/workspace/astroclip/results/astroclip_image/{run_name}"
    os.environ["DINOv2_DISTRIBUTED_DISABLE"] = "1"
    cfg = custom_setup(args)

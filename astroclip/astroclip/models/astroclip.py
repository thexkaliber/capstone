...
class AstroClipModel(L.LightningModule):
    def __init__(
        ...
        self.logit_scale_param = logit_scale
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_nologit = self.criterion(
            image_features, spectrum_features, self.logit_scale
        )
...
class CLIPLoss(nn.Module):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.epochs, eta_min=self.eta_min)
        return {"optimizer": optimizer, "lr_scheduler": {"scheduler": scheduler, "monitor": "val_loss_withlogit"}}
...
class ImageHead(nn.Module):
    def __init__(
        ...
        self.backbone, _ = setup_and_build_model(config(), model_name=model_name)
    ...
    def forward(self, x: torch.tensor, return_weights: bool = False):
        ...
        with torch.set_grad_enabled(not self.freeze_backbone):
            try:
                x_astrodino = self.backbone.patch_embed(x)
                for blk in self.backbone.blocks:
                    x_astrodino = blk(x_astrodino)
                embedding = self.backbone.norm(x_astrodino)
                
            except:
                output = self.backbone(x, masks=None, is_training=True)
                embedding = output["x_norm_patchtokens"]
...
class ImageHeadSwin(nn.Module):
    def __init__(
        ...
        if model_name == "swin" or model_name=="swinv2":
            backbone_output_dim = 2048
        else:
            backbone_output_dim = 1024
        
        if backbone_output_dim != model_embed_dim:
            print("Output dim mismatch Cross-Attention shape, projecting to fit.")
            self.projection = nn.Linear(backbone_output_dim, model_embed_dim)

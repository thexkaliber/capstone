...
Enhanced with variable image crop size support and masking logic from DinoV2 Vision Transformer
"""
import math
import logging
...
class XCiT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        # Initialize positional embeddings for learned pos encoding (fallback when Fourier is disabled)
        ...
        # Mask token for masking support
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        npatch = x.shape[1] - 1  # number of patches (excluding cls token)
        N = self.pos_embed.shape[1] - 1  # number of patches in pretrained pos_embed
        if npatch == N and w == h:
        ...
        assert N == M * M, f"Pretrained pos_embed expects square grid, got {N} patches"
    ...
    def prepare_tokens_with_masks(self, x, masks=None):
        ...
        if self.use_pos:
            # Use Fourier positional encoding for current patch grid
            pos_encoding = self.pos_embeder(B, Hp, Wp).reshape(B, -1, x.shape[1]).permute(0, 2, 1)
            x = x + pos_encoding
        else:
            # Use learned positional encoding with interpolation
            # Create a dummy tensor with cls token for interpolation
            x_with_cls = torch.cat((self.cls_token.expand(B, -1, -1), x), dim=1)
            pos_embed = self.interpolate_pos_encoding(x_with_cls, w, h)
            x = x + pos_embed[:, 1:]  # exclude cls token position
    ...
    def forward_features_list(self, x_list, masks_list):
        """
        Forward pass through feature extraction layers for lists of inputs with corresponding masks.
        Similar to DinoV2 Vision Transformer implementation.
        """
        # Prepare all inputs with their masks
        prepared_inputs = []
        patch_shapes = []
        for x, masks in zip(x_list, masks_list):
            B, nc, w, h = x.shape
            x_prepared, (Hp, Wp) = self.prepare_tokens_with_masks(x, masks)
            prepared_inputs.append(x_prepared)
            patch_shapes.append((Hp, Wp))

        # Process through XCA blocks
        for blk in self.blocks:
            for i in range(len(prepared_inputs)):
                Hp, Wp = patch_shapes[i]
                prepared_inputs[i] = blk(prepared_inputs[i], Hp, Wp)

        # Add cls tokens and apply class attention blocks
        outputs_with_cls = []
        for i, x in enumerate(prepared_inputs):
            B = x.shape[0]
            ...
            for blk in self.cls_attn_blocks:
                x_with_cls = blk(x_with_cls, Hp, Wp)
            
            outputs_with_cls.append(x_with_cls)

        # Normalize and prepare output
        output = []
        for x, masks in zip(outputs_with_cls, masks_list):
            x_norm = self.norm(x)
            output.append({
    ...
    def forward_features(self, x, masks=None):
        """
        Forward pass through the feature extraction layers with optional masking.
        Enhanced to support variable image sizes and masking.
        """
        if isinstance(x, list):
            return self.forward_features_list(x, masks)

        B, nc, w, h = x.shape
        x, (Hp, Wp) = self.prepare_tokens_with_masks(x, masks)

        # XCA blocks
        for blk in self.blocks:
            x = blk(x, Hp, Wp)

        # Add cls token and apply class attention blocks
        cls_tokens = self.cls_token.expand(B, -1, -1)
    ...
    def forward(self, x, masks=None, is_training=False):
        """
        Enhanced forward method with masking support and training/inference modes.
        """
        ret = self.forward_features(x, masks)
        if is_training:
            return ret
        else:
            x = self.head(ret["x_norm_clstoken"])
            if self.training:
                return x, x
            else:
                return x
    ...
    def get_intermediate_layers(
        ...
        """
        B, nc, w, h = x.shape
        x, (Hp, Wp) = self.prepare_tokens_with_masks(x)
        ...
        if reshape:
            output = [
                out.reshape(B, Hp, Wp, -1).permute(0, 3, 1, 2).contiguous()
                for out in output
            ]
        
        if return_class_token:
            # For XCiT, we need to add class tokens and pass through class attention
            output_with_cls = []
            for out in output:
                cls_tokens = self.cls_token.expand(B, -1, -1)
                out_with_cls = torch.cat((cls_tokens, out), dim=1)
                for blk in self.cls_attn_blocks:
                    out_with_cls = blk(out_with_cls, Hp, Wp)
                if norm:
                    out_with_cls = self.norm(out_with_cls)
                class_tokens = out_with_cls[:, 0]
                patch_tokens = out_with_cls[:, 1:]
                if reshape:
                    patch_tokens = patch_tokens.reshape(B, Hp, Wp, -1).permute(0, 3, 1, 2).contiguous()
                output_with_cls.append((patch_tokens, class_tokens))
            return tuple(output_with_cls)

...
class SSLMetaArch(nn.Module):
    def __init__(self, cfg):
        ...
        if self.do_dino:
            ...
            final_embed_dim = cfg.student.embed_dim * 2 ** (len(cfg.student.depths) - 1)
            dino_head = partial(
                DINOHead,
                in_dim=final_embed_dim, #embed_dim
    ...
    def forward_backward(self, images, teacher_temp):
        with torch.cuda.amp.autocast(enabled=self.fp16_scaler is not None):
            ...
            def get_teacher_output():
                ...
                total_patches = ibot_teacher_patch_tokens.numel() // ibot_teacher_patch_tokens.shape[-1]
                if mask_indices_list.max().item() >= total_patches:
                    mask_indices_list = torch.randperm(total_patches, device=ibot_teacher_patch_tokens.device)[:mask_indices_list.shape[0]]

                    assert mask_indices_list.max().item() < total_patches, f"mask_indices contains out-of-bounds indices: max={mask_indices_list.max().item()}, allowed={total_patches-1}"                

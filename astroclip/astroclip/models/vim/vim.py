...
class Block(nn.Module):
    def __init__(
        self, dim, mixer_cls, norm_cls=nn.LayerNorm, fused_add_norm=False, residual_in_fp32=False,drop_path=0., **kwargs
    ):
        """
...
def segm_init_weights(m):
    if isinstance(m, nn.Linear):
        ...
        # NOTE conv was left to pytorch default in original init
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        self.use_masked_modeling = use_masked_modeling
        ...
        self.layers = nn.ModuleList(
            [
                create_block(
                    ...
                    bimamba_type=bimamba_type if if_bimamba else "none",
    ...
    def interpolate_pos_encoding(self, x, w, h):
        """
        nterpolation functionality is essential for handling variable input sizes
        """
        npatch = x.shape[1] - 1
    ...
    def prepare_tokens_with_masks(self, x, masks=None):
        ...
        # Apply masks for masked image modeling
        if masks is not None:
            if masks.dtype != torch.bool:
                masks = masks.bool()
            
            # Only apply masking if dimensions match naturally
            if masks.shape[1] == x.shape[1]:
                x = torch.where(masks.unsqueeze(-1), self.mask_token.expand(B, -1, -1), x)

        # Handle CLS token insertion consistently
        token_position = None
        original_patch_count = x.shape[1]  # Store original count before CLS insertion
        ...
        return x, token_position, original_patch_count
    ...
    def forward_features_list(self, xs, masks_list=None, inference_params=None, 
                            if_random_cls_token_position=False, if_random_token_rank=False):
        if masks_list is None:
            masks_list = [None] * len(xs)

        tokens_list = []
        token_positions_list = []
        original_patch_counts = []
        
        for x, masks in zip(xs, masks_list):
            tokens, token_positions, original_patch_count = self.prepare_tokens_with_masks(x, masks)
            tokens_list.append(tokens)
            token_positions_list.append(token_positions)
            original_patch_counts.append(original_patch_count)
        ...
        output = []
        for i, (x_norm, masks, token_positions, original_patch_count) in enumerate(
            zip(processed, masks_list, token_positions_list, original_patch_counts)
        ):
            # Extract CLS and patch tokens based on configuration
            if self.if_cls_token:
                if self.use_double_cls_token:
                    cls_token = (x_norm[:, 0] + x_norm[:, -1]) / 2
                    patch_tokens = x_norm[:, 1:-1]
                elif self.use_middle_cls_token:
                    if isinstance(token_positions, int):
                        cls_token = x_norm[:, token_positions]
                        # Reconstruct patch tokens in original order
                        patch_tokens = torch.cat([
                            x_norm[:, :token_positions], 
                            x_norm[:, token_positions+1:]
                        ], dim=1)
                    else:
                        cls_token = x_norm[:, 0]
            ...
            output.append({
                ...
                'x_norm_regtokens': torch.empty(cls_token.size(0), 0, cls_token.size(1), 
    ...
    def forward_features(self, x, masks=None, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False,
                         return_intermediate_output=False, is_training=False):
        ...
        x, token_position, original_patch_count = self.prepare_tokens_with_masks(x, masks)
        B, M, _ = x.shape
        ...
        # NOTICE: is_training path requires discussion before modification
        if is_training:
            # Extract tokens for training
            if self.if_cls_token:
                if self.use_double_cls_token:
                    cls_token = (hidden_states[:, 0] + hidden_states[:, -1]) / 2
                    patch_tokens = hidden_states[:, 1:-1]
                elif self.use_middle_cls_token:
                    cls_token = hidden_states[:, token_position]
                    # Reconstruct patch tokens in original order
                    patch_tokens = torch.cat([
    ...
    def forward(self, x, masks: Optional[Tensor] =None, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False, is_training=False):
        ...
        hidden_states = self.forward_features(x, masks, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank, return_intermediate_output=False, is_training=is_training)
